...
function render() {
  ...
  const titles = { itinerary: ['A LITTLE GETAWAY', '杭州，慢慢走。', '留一点空白，给路上的惊喜。'], ledger: ['EVERY LITTLE MOMENT COUNTS', '一起出发，一起记。', '花得明白，玩得轻松。'], media: ['MEMORIES, WITH YOUR SIGNATURE', '把回忆，好好收着。', '一路的风景，都是我们的故事。'] };
...
$('#preferences-form').addEventListener('submit', (event) => {
  ...
  $('#proposal').innerHTML = `<h3>留住想去的，也留一点空白。</h3><ul>${changes.map(text => `<li>${esc(text)}</li>`).join('')}</ul><p class="form-note">这是本地规则演示，不含实时搜索、价格校验或真实 Agent 调用。预算与住宿偏好已记录，暂未据此查找酒店和核算票价。</p><button class="primary-button" data-action="accept-proposal">采纳这份草案</button>`;
