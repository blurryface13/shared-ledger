...
(() => {
  ...
  window.resetLedgerPanel=()=>{section='bills';filter='';editing=null;exportScope='book';};
  ...
  function model() {
    state.ledger ||= { name: '杭州两日', budget:4000, start:'2026-09-26', categories: ['餐饮','交通','住宿','门票','其他'], roles: { 我:'所有者', 小林:'管理员', 小陈:'成员', 小余:'成员' }, invitations: [], temps: [], requests: [], payments: [], exports: [], logs: [], archived: false };
    return state.ledger;
  }
  ...
  documentHtml = function() {
    ...
    return `<div class="paper-brand">SHARED LEDGER</div><div class="paper-title">${esc(model().name)} · ${exportScope==='personal'?'个人明细':'账本汇总'}</div><p class="paper-meta">${bills.length} 笔 · 本地演示</p><section class="paper-day"><b>${exportScope==='personal'?'我的承担金额':'账本总支出'} ¥${money(exportScope==='personal'?bills.reduce((s,b)=>s+(shares(b).find(x=>x.name==='我')?.cents||0),0):total())}</b>${bills.map(b=>`<div class="paper-row"><span>${dateLabel(billDate(b)).slice(5)}</span><div>${esc(b.title)} · ¥${money(b.cents)}<small>${esc(b.payer)}先付 / ${shares(b).map(x=>`${esc(x.name)} ¥${money(x.cents)}`).join('、')}</small></div></div>`).join('')}</section><section class="paper-day"><b>待结算建议</b>${transfers().map(t=>`<div class="paper-row"><span>转账</span><div>${esc(t.from)} → ${esc(t.to)} ¥${money(t.cents)}</div></div>`).join('')||'<p class="paper-meta">账目已平。</p>'}</section><p class="paper-meta" style="margin-top:18px">不包含私密凭据。结算建议不代表实际付款。</p>`;
  };
  render = function() {
  ...
  openBill=function(activity=null) {
    ...
    editing=null;oldOpenBill(activity);const form=$('#bill-form');if(!activity) form.elements.date.value=new Date().toLocaleDateString('en-CA');
  ...
  window.saveLedgerBill = function(form) {
    ...
    const next={id:editing||crypto.randomUUID(),title,cents,participants,payer:data.get('payer'),category:data.get('category'),day:0,date:data.get('date'),activityId:linkedActivity};
  ...
  document.addEventListener('submit',event=>{
    ...
    if(id==='book-settings'){model().name=name;model().budget=Number(data.get('budget'));log('更新账本设置');}
