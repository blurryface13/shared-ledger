...
public class TripApplicationService {
    ...
    public TripApplicationService(TripRepository repository,BookApplicationService books,MapGateway maps){this.repository=repository;this.books=books;this.maps=maps;}
    ...
    @Transactional
    public Trip create(long user,Trip trip){trip.validate();binding(user,trip.bookId());return repository.create(user,trip.identified(null,0));}
    @Transactional
    public Trip update(long user,long id,Trip next){Trip before=get(user,id);if(before.version()!=next.version())throw new BusinessException(ErrorCode.CONFLICT,"行程已被修改，请刷新后重试");next=next.identified(id,next.version());next.validateChange(before);if(!Objects.equals(next.bookId(),before.bookId())){binding(user,before.bookId());binding(user,next.bookId());}return repository.update(user,next);}
