...
public class TripApplicationService {
    ...
    public MapGateway.Center center(long user,long id){return maps.center(get(user,id).destination());}
    public List<MapGateway.Place> nearby(long user,long id,double longitude,double latitude,String category){
        get(user,id); // Access is checked before a provider request, including for read-only members.
        Trip.require(Double.isFinite(longitude)&&Double.isFinite(latitude)&&Math.abs(longitude)<=180&&Math.abs(latitude)<=90,"坐标无效");
        Trip.require(Set.of("sights","stays","food").contains(category==null?"":category),"地点类型无效");
        return maps.nearby(longitude,latitude,category);
    }
